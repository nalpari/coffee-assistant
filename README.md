...
### 📍 매장 검색 및 위치 기반 서비스
- **위치 기반 매장 검색**: 사용자 위치 기반 가까운 매장 자동 정렬
- **실시간 거리 계산**: 각 매장까지의 거리 실시간 표시
- **매장 상세 정보**: 매장 정보, 메뉴, 리뷰, 공지사항 통합 제공
- **영업 상태 필터링**: 영업 중인 매장만 보기 옵션
- **다양한 정렬 옵션**: 거리순, 인기순, 최신순 정렬
...
- `menu_items` - 메뉴 아이템
- `stores` - 매장 정보
- `store_menus` - 매장별 메뉴 매핑
- `orders` - 주문 정보
- `order_items` - 주문 상세
- `users` - 사용자 정보

**주요 테이블 필드:**
- `stores`: id, name, description, latitude, longitude, address, phone, status, image, notice 등
- `store_menus`: store_id, menu_item_id, is_available, stock_quantity 등
...
│   │   │   ├── StoreCard.tsx     # 매장 카드
│   │   │   ├── StoreInfo.tsx     # 매장 정보
│   │   │   ├── StorePhotoSection.tsx # 매장 사진
│   │   │   ├── StoreMenuList.tsx # 매장 메뉴 목록
│   │   │   └── StoreSearchHeader.tsx # 검색 헤더
│   │   ├── orders/               # 주문 컴포넌트
│   │   │   ├── OrderList.tsx
│   │   │   └── OrderCard.tsx
│   │   ├── chat/                 # AI 채팅 UI
│   │   │   ├── ChatInput.tsx
│   │   │   ├── ChatMessage.tsx
│   │   │   └── DuplicateOrderDialog.tsx # 중복 주문 확인
│   │   ├── ai/                   # AI 관련 컴포넌트
│   │   │   └── StoreSelectionCard.tsx # 매장 선택 카드
│   │   ├── layout/               # 레이아웃 컴포넌트
│   │   │   ├── Header.tsx
...
│   │   ├── duplicate-order-detector.ts # 중복 주문 감지
│   │   ├── order-utils.ts        # 주문 처리 유틸리티
│   │   ├── payment-utils.ts      # 결제 유틸리티
│   │   ├── price-utils.ts        # 가격 계산 유틸리티
│   │   ├── location-utils.ts     # 위치 및 거리 계산
│   │   ├── menu-search-utils.ts  # 메뉴 검색 유틸리티
│   │   ├── store-menu-utils.ts   # 매장 메뉴 유틸리티
│   │   └── api/
│   │       ├── menu.ts           # 메뉴 API 함수
│   │       └── stores.ts         # 매장 API 함수
│   │
...
│   │   ├── use-stores-query.ts   # 매장 데이터 페칭
│   │   ├── use-store-menu-query.ts # 매장 메뉴 데이터 페칭
...
│   │   └── GeolocationContext.tsx # 위치 정보 컨텍스트
│   │
│   └── data/                     # 목 데이터 및 상수
...
사용자 위치 요청
    ↓
GeolocationContext (위치 정보 수집)
    ↓
Stores API 호출 (위치 기반 정렬)
    ↓
거리 계산 및 정렬
    ├─ calculateDistance: 하버사인 공식 사용
    ├─ formatDistance: 거리 포맷팅 (km/m)
    └─ 정렬 옵션 적용 (거리순/인기순/최신순)
    ↓
매장 목록 표시
```
...
- **서버 상태**: TanStack Query (menu, orders, products, stores)
- **인증 상태**: React Context + Supabase
- **위치 상태**: GeolocationContext + Browser Geolocation API
